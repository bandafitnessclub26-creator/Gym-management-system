    ...
    <div className="min-h-screen bg-black pb-24">
      <div className="bg-gradient-to-b from-zinc-900 to-black pt-12 pb-6 px-6">
        <div className="flex items-center gap-4 mb-6">
          <button
            onClick={() => navigate(-1)}
            className="w-10 h-10 bg-zinc-800 rounded-xl flex items-center justify-center text-white"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <h1 className="text-2xl text-white">Add Fee Payment</h1>
      ...
      <div className="px-6">
        {member && (
          <Card className="bg-zinc-900 border-zinc-800 rounded-2xl mb-6">
            <CardContent className="p-5">
              <p className="text-white text-lg font-bold">{member.name}</p>
              <p className="text-zinc-400 text-sm">{member.plan}</p>
              <p className="text-[#39FF14] text-xl mt-2">
                ₹{monthlyFee.toLocaleString()}
              </p>
            </CardContent>
          </Card>
        ...
        <Card className="bg-zinc-900 border-zinc-800 rounded-2xl">
          <CardContent className="p-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {!memberId && (
                <div>
                  <Label className="text-zinc-300">Search Member</Label>
                  <Input
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full bg-[#39FF14] text-black"
              >
                {loading ? (
